...
<!-- Tier-2 standard **Eval-Hardening Slice** template (SmartForge V2 Tier-2 methodology).
     ...
     it is carved to the secure-coding profile (SP4 / CR-009). The "Security posture" NFR
...
- Injection / jailbreak review: confirm the agent never leaks its system/rating prompt and never
  executes instructions embedded in untrusted input. (Secure-coding declaration → SP4 / CR-009;
  reference it — do not rebuild the secure-coding activity here.)
...
- [ ] Injection / jailbreak review passed — no system/rating prompt leakage, no untrusted-input
      instruction execution detected; secure-coding declaration confirmed routed to SP4 / CR-009.
...
- Security posture: <injection / jailbreak resistance declared here; the secure-coding review + threat-model activity is SP4 / CR-009, not this slice — reference that profile, do not rebuild it>
